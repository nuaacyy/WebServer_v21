		...
		if(headers.containsKey(HttpContext.HEADER_CONTENT_LENGTH)) {
			...
			try {
				...
				if("application/x-www-form-urlencoded".equals(contentType)) {
					//开始处理form表单数据
					System.out.println("开始处理form表单数据");
					//读取正文中的所有字节
...
//					 */
	...
	private void parseQueryString(String line) {
		try {
			...
			System.out.println("长度为："+paraArray.length);
...
//			// 1 按照“&”拆分出每个参数
			...
			//打桩
			Set<Entry<String, String>> entry=parameters.entrySet();
			for (Entry<String, String> entry2 : entry) {
				System.out.println("parameters中的键值对为：");
